<script lang="ts">
	...
	import { hideUserInputPrompt, userInputStore } from "$lib/stores/userInputStore";

	let userInput = "";

	async function handleSubmit() {
		const run_id = $userInputStore.run_id;
		if (run_id) {
...
{#if $userInputStore.isWaiting}
	<div class="rounded-lg border bg-card p-4 text-card-foreground shadow-sm">
		...
		<form on:submit|preventDefault={handleSubmit} class="space-y-4">
			<Input type="text" bind:value={userInput} placeholder="Enter your response" class="w-full" />
