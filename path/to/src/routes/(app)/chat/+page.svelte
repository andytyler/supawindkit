<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    const systemPrompt =
      "You are a helpful assistant. You have access to snippets in user input. Use them to answer the user's question."
    ...
    >
      {#each messages as message}
        ...
        >
          ...
          >
            <!-- Render sanitized message content as Markdown -->
            {@html sanitize(message.content)}

            {#if message.snippets && message.snippets.length > 0}
              <div
                class="mt-2 p-2 bg-yellow-100 border-l-4 border-yellow-500 rounded"
              >
                <h4 class="font-semibold text-yellow-700">Snippets Context:</h4>
                <ul class="list-disc list-inside">
                  {#each message.snippets as snippet}
                    <li class="text-sm text-yellow-600">
                      <!-- Render sanitized snippet content as Markdown -->
                      {@html sanitize(snippet.content)}
                      <span class="text-xs text-gray-500">
                        ({snippet.source || "Unknown Source"})
                      </span>
                    </li>
                  {/each}
                </ul>
